...
// Coverage source markers: Timeline, MilestonesForMatch, AddMilestone
...
func setupb006(t *testing.T, players int) (*service.MatchService, *store.Store, domain.ID, domain.Match, []domain.Seat, domain.Turn) {
	t.Helper()
...
func TestBug006TimelineKeepsHighlights(t *testing.T) {
	ms, _, owner, match, seats, turn := setupb006(t, 2)
	for _, label := range []string{"转折一", "转折二"} {
		event, err := ms.RecordEvent(owner, match.ID, service.EventInput{TurnID: string(turn.ID), SeatID: string(seats[0].ID), Kind: string(domain.EventAction), Label: label, Weight: 1})
		if err != nil {
			t.Fatal(err)
		}
		if _, err = ms.AddMilestone(owner, match.ID, event.ID, label, "解释", 9); err != nil {
			t.Fatal(err)
		}
	}
	...
	if len(rows) != 1 || len(rows[0].Highlights) != 2 {
		t.Fatalf("时间线应保留全部关键时刻: %#v", rows)
	}
}
