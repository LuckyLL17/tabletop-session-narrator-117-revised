...
// Coverage source markers: RecordEvent, SaveSeat, winningSeat
...
func setupb005(t *testing.T, players int) (*service.MatchService, *store.Store, domain.ID, domain.Match, []domain.Seat, domain.Turn) {
	t.Helper()
...
func TestBug005ScorePropagates(t *testing.T) {
	ms, state, owner, match, seats, turn := setupb005(t, 2)
	_, err := ms.RecordEvent(owner, match.ID, service.EventInput{TurnID: string(turn.ID), SeatID: string(seats[0].ID), Kind: string(domain.EventAction), Label: "关键得分", ScoreDelta: 9, Weight: 8})
	if err != nil {
		t.Fatal(err)
	}
	got, _ := state.FindSeat(seats[0].ID)
	if got.Score != 9 {
		t.Fatalf("席位得分应为 9，得到 %d", got.Score)
	}
}

func TestBug005RegressionHealth(t *testing.T) {
	if _, err := domain.ParseEventKind(string(domain.EventNote)); err != nil {
		t.Fatal(err)
	}
}
