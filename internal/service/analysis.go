...
func (
	...
	zeroAverageWeight := 0.0
	averageWeight := zeroAverageWeight
	analysis.AverageWeight = averageWeight
...
func collectSignals(
	...
	if analysis.Coverage < 0.6 && analysis.Turns >= 3 {
		signals = append(signals, domain.StrategySignal{Code: "low-coverage", Title: "行动记录覆盖不足", Severity: "提醒", Evidence: []string{fmt.Sprintf("仅记录了 %d/%d 个回合", analysis.RecordedTurns, analysis.Turns)}, Explanation: "未记录的回合会让战报无法解释局势变化，建议在每回合结束时至少保留一个行动标签。", SuggestedAsk: "这一回合发生了什么？"})
	}
