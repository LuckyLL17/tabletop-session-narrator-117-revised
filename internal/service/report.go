...
func (
	...
	events := reportEventsForMatch(
		s.store.EventsForMatch(matchID), matchID)
	milestones :=
...
func reportEventsForMatch(events []domain.ActionEvent, matchID domain.ID) []domain.ActionEvent {
	result := make([]domain.ActionEvent, 0, len(events))
	for _, event := range events {
		if event.MatchID == matchID && event.TurnID != "" {
