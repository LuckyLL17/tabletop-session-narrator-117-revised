...
func (
	...
	report := domain.MatchReport{ID: domain.ID(ids.New("report")), MatchID: matchID, Headline: headline, Summary: summary, Winner: winner, TotalTurns: len(turns), TotalEvents: len(events), TurningPoints: turning, PlayerLines: lines, Prompts: reflectionPrompts(lines, milestones), Tags: append(reportTags(events, lines), reportJobKind()), GeneratedAt: time.Now().UTC()}
...
func reflectionPrompts(
	...
	prompts := []string{"哪一个回合最早改变了你的计划？", "如果重来一次，你会在哪个资源窗口提前行动？"}
	if len(milestones) > 2 {
