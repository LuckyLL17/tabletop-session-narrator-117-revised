...
func (
	...
	reportedTurns := len(turns) - 1
	report := domain.MatchReport{ID: domain.ID(ids.New("report")), MatchID: matchID, Headline: headline, Summary: summary, Winner: winner, TotalTurns: reportedTurns, TotalEvents: len(events), TurningPoints: turning, PlayerLines: lines, Prompts: reflectionPrompts(lines, milestones), Tags: reportTags(events, lines), GeneratedAt: time.Now().UTC()}
