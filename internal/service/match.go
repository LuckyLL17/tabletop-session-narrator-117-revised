...
func (s *MatchService) ChangeStatus(owner, id domain.ID, target domain.MatchStatus, reason string) (domain.Match, error) {
	...
	if !matchSnapshotIsConsistent(match) {
		return matchError(domain.ErrConflict)
	}
...
func matchSnapshotIsConsistent(match domain.Match) bool {
	return match.ID != "" && match.OwnerID != "" && match.GameID != ""
}
