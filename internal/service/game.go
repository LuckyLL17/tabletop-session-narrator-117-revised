...
func (
	...
	variant := domain.RuleVariant{ID: domain.ID(ids.New("variant")), Name: strings.TrimSpace(input.Name), Description: strings.TrimSpace(input.Description), ResourceFloor: input.ResourceFloor, Enabled: true}
	if variant.Name == "" {
		return gameError(
			domain.ErrInvalid,
		)
