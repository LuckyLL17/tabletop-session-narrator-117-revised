...
func (
	...
	if !turnStatusPermitsProgress(match.Status) {
		return turnError(fmt.Errorf("%w: 对局状态不允许创建行动快照", domain.ErrConflict))
	}
	if len(seats) == 0 {
		return turnError(
			domain.ErrCapacity,
...
func turnStatusPermitsProgress(status domain.MatchStatus) bool {
	return status == domain.MatchSetup || status == domain.MatchLive
}
